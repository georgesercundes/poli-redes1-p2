...
def baixar():
    with connection:
        ...
        with open('{}/{}.mp3'.format(diretorio, musica[2:-1]), 'rb') as bytes:
            connection.send(struct.pack('i', os.path.getsize('{}/{}.mp3'.format(diretorio, musica[2:-1]))))
...
def enviar():
    with connection:
        musica = str(connection.recv(bufferSize))
        with open('{}/{}.mp3'.format(diretorio, musica[2:-1]), 'wb') as arquivoMusica:
