...
def enviar (musica):
    with socketClient:
        with open('{}/{}'.format(diretorio2, '{}.mp3'.format(musica)), 'rb') as bytes:
            socketClient.send(musica.encode('utf-8'))
            socketClient.send(struct.pack('i', os.path.getsize('{}/{}'.format(diretorio2, '{}.mp3'.format(musica)))))
...
def baixar(musica):
    with socketClient:
        with open('{}/{}'.format(diretorio, '{}.mp3'.format(musica)), 'wb') as arquivoMusica:
            socketClient.send(musica.encode('utf-8'))
...
def tocar(musica):
    with socketClient:
        with open('{}/{}'.format(diretorio3, '{}.mp3'.format(musica)), 'wb') as arquivoMusica:
            socketClient.send(musica.encode('utf-8'))
            fileSize = struct.unpack('i', socketClient.recv(4))[0]
            bytesReceived = 0
            while bytesReceived < fileSize:
    ...
    playsound('{}/{}'.format(diretorio3, '{}.mp3'.format(musica)))
    return
...
while True:
    ...
    if request == 'enviar':
        listarMusicasUpload()
        musica = input('Musica: ')
        if (os.path.exists('{}/{}'.format(diretorio2, '{}.mp3'.format(musica))) == True):
            socketClient.send(request.encode('utf-8'))
            time.sleep(1)
            enviar(musica)
            socketClient = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
